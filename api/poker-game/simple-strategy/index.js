...
const rule = (rule_number, INPUT_HAND) => {
  ...
  const score = Poker.Score(INPUT_HAND);
    ...
    case 14:
      ...
      if (high.cards.length === 1) {
        const cardT = INPUT_HAND.filter((card) =>
          cardValue(card) === VALUES_INDEX[9] ? card : false
        );
        ...
        if (cardT[0]) {
          ...
          if (tSuitedHighCards.length > 0) {
            // No Aces
            if (cardValue(tSuitedHighCards[0]) !== VALUES_INDEX[0]) {
              return {
                rule_number,
...
const simpleStrategy = (THE_CARDS) => {
